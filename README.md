...
Tabulation System for Queen of San Antonio (Iriga City)
...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/qosa-iriga`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `qosa-iriga` and import [qosa-iriga.sql](qosa-iriga.sql) into it.

6. Open the terminal and navigate to the project directory **qosa-iriga**.
...
9. Open your web browser and access <http://localhost:5178/qosa-iriga/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-qosa-iriga` and import [qosa-iriga.sql](qosa-iriga.sql) into it.
...
2. Access the application by visiting `http://[host_name]/qosa-iriga`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/qosa-iriga>
     - <http://192.168.1.99/qosa-iriga>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/qosa-iriga/app>.
